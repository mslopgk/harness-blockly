# -*- coding: utf-8 -*-
"""[H2] 텔레오퍼레이션 (제스처 전기능 제어) GUI — 단일 파일(standalone) 버전.

원본(taskB_curriculum/h2_teleop/gui.py + 의존 모듈들: common.config /
common.safety / common.vision(MediaPipe HandTracker 포함) / common.dobot_ctl /
common.gui / h2_teleop.solution)을 **하나의 파이썬 파일**로 합친 것.

자동 생성물 — 로직은 원본과 동일하다(중복 구현/재작성 아님). 여러 모듈을 한 네임스페이스로
평탄화하고, config./safety./solution. 같은 모듈 한정 참조가 이 파일 자신을 가리키도록
alias 처리했다.

실행:  python 텔레오퍼레이션_standalone.py     (하드웨어 없으면 자동 mock)
필요:  tkinter(표준), 선택적으로 opencv-python / pillow / numpy / mediapipe / pydobot.
"""
...
# ── 패키지 평탄화용 self-alias ───────────────────────────────────────────────
# 원본은 common / h2_teleop 패키지로 나뉘어 `config.X` / `safety.X` /
# `solution.X` / `_config` / `_accel` 처럼 모듈을 한정해 서로를 참조했다. 단일
# 파일에서는 그 이름들이 모두 '이 모듈'을 가리키게 하면, 아래에 이어붙인 각 원본
# 코드의 참조가 그대로 동작한다.
...
solution = _sys.modules[__name__]   # solution.estimate_affine / apply_affine ...
_config  = _sys.modules[__name__]   # common.vision 내부 별칭
_accel   = None                     # 선택적 GPU 가속(accel) 비활성 → 순수 cv2 경로

# numpy: 아핀 캘리브레이션(estimate_affine)의 핵심(없으면 안내).
try:
    import numpy as np
    _HAS_NUMPY = True
except Exception:
...
# ==============================================================================
# ── common/vision.py — Camera + HandTracker(MediaPipe)
# ==============================================================================
...
# ==============================================================================
# ── h2_teleop/solution.py — 아핀 순수함수 + teleop
# ==============================================================================

"""H2 · 고등부(Python) — 텔레오퍼레이션: 좌표계 선형 변환 [모범답안]

수업 목표 (110분 · 5E · Logic-First, Syntax-Later · 산업연계 PBL)
----------------------------------------------------------------
MediaPipe 손 추적으로 얻은 **카메라(픽셀) 좌표**를 **로봇 말단(데카르트) 좌표**로
선형 변환(affine)하여, 손을 움직이면 로봇팔이 따라오는 *텔레오퍼레이션*을 만든다.

핵심 개념(선형대수)
  - 캘리브레이션: 카메라에서 본 점 ↔ 로봇에서의 실제 점을 N쌍(대응점) 모아
    아핀 변환행렬 M(2x3)을 **최소제곱(numpy.linalg.lstsq)** 으로 추정한다.
        [rx, ry]^T = M · [cx, cy, 1]^T
  - 매핑: 추정된 M으로 임의의 카메라 점을 로봇 (x, y)로 사상한다.
  - 좌표계: 픽셀 좌표(원점 좌상단, y 아래로 증가) vs 로봇 좌표(mm) 의 방향/스케일 차이를
    행렬 한 장이 흡수한다는 것을 체험.

산업 연계
  실제 비전 가이드 로봇(픽앤플레이스, 수술보조, 원격조종)의 'hand-eye calibration'
  축소판. 카메라 픽셀을 로봇 mm로 바꾸는 변환행렬은 산업현장의 표준 절차다.

설계 특이사항 (단위테스트 가능)
  - estimate_affine(대응점)→행렬, apply_affine(행렬, 점)→로봇점 을 **순수함수**로 분리.
    (입력만으로 출력이 결정 → 카메라/로봇 없이 pytest로 검증 가능)

안전 (QUR-002: 관절 무한루프·좌표 초과·충돌 없음)
  손 위치가 튀어 작업영역 밖 좌표가 나와도 RobotController.move_to 가 내부에서
  safety.clamp_move 로 보정한다. 추가로 데모 시작 전 safety.check_move 로 1차 확인.

실행 (하드웨어/키 없이 mock 으로 끝까지)
  CURRICULUM_FORCE_MOCK=1 python h2_teleop/solution.py
"""

# --- sys.path 부트스트랩(2줄): 이 파일 위치의 상위(taskB_curriculum)를 import 경로에 추가 ---
import os, sys  # noqa: E401  (부트스트랩 관용구)

# Windows 콘솔(cp949)에서도 한국어/유니코드 출력이 깨지지 않도록 표준출력을 UTF-8로.
try:
    sys.stdout.reconfigure(encoding="utf-8")   # Python 3.7+
except Exception:
    pass

# 공통 라이브러리 (CONTRACTS.md 시그니처)
pass
pass
pass
pass

# numpy: 선형대수(아핀 추정/적용)의 핵심. 없으면 친절히 안내(이 차시는 numpy 필수).
try:
    import numpy as np            # 행렬/최소제곱 연산
    _HAS_NUMPY = True
except Exception:                 # numpy 미설치 환경 방어
    np = None
    _HAS_NUMPY = False


# ===========================================================================
# 1) 순수함수: 아핀 변환행렬 추정 (캘리브레이션)
#    - 입력: 대응점 쌍 [(카메라점, 로봇점), ...]  (카메라점=(cx,cy), 로봇점=(rx,ry))
#    - 출력: 2x3 아핀 행렬 M  (numpy.ndarray)
#    - 부수효과 없음 → 단위테스트 가능
# ===========================================================================
...
# ===========================================================================
# 2) 순수함수: 아핀 변환 적용 (카메라 점 → 로봇 점)
#    - 입력: 행렬 M(2x3), 카메라 점 (cx, cy)
#    - 출력: 로봇 점 (rx, ry)   (float, mm)
# ===========================================================================
...
# ===========================================================================
# 3) 순수함수: 손 랜드마크 → 카메라 픽셀 좌표
#    MediaPipe 랜드마크는 정규화(0~1) 좌표 → 프레임 폭/높이로 픽셀 환산.
#    검지 끝(INDEX_TIP=8)을 손 '포인터'로 사용한다.
# ===========================================================================
def hand_to_pixel(landmarks, frame_w, frame_h, tip=HandTracker.INDEX_TIP):
    """손 랜드마크에서 포인터(검지끝) 픽셀 좌표를 뽑는다(순수함수).

    인자
      landmarks: [(x,y,z), ...] 정규화(0~1) 21점. 없으면 None 반환.
      frame_w, frame_h: 프레임 가로/세로 픽셀.
      tip: 사용할 랜드마크 인덱스(기본 검지 끝).
    반환
      (px, py) 픽셀 좌표 또는 None(손 없음).
    """
    # 손을 못 찾았거나 점 수가 부족하면 포인터 없음
    if not landmarks or len(landmarks) <= tip:
        return None
    nx, ny, _ = landmarks[tip]           # 정규화 좌표(0~1)
    px = nx * frame_w                    # 가로 픽셀 = 정규화x * 폭
    py = ny * frame_h                    # 세로 픽셀 = 정규화y * 높이
    return px, py


# ===========================================================================
# 4) 캘리브레이션 데이터 — 카메라 코너 4점 ↔ 로봇 작업영역 코너 4점
#    실제 수업에선 학생이 로봇을 코너로 보내며 GetPose로 로봇점을 입력한다.
#    여기선 작업영역(WORKSPACE) 안전범위로 결정적 대응표를 구성(재현성).
# ===========================================================================
def default_correspondences(frame_w=640, frame_h=480):
    """카메라 프레임 네 모서리 ↔ 로봇 작업영역 네 모서리 대응표를 만든다.

    카메라: (0,0)=좌상, (W,0)=우상, (0,H)=좌하, (W,H)=우하
    로봇  : config.WORKSPACE 의 x/y 범위 안쪽으로 매핑(여유 마진).
      - 카메라 x(가로) → 로봇 y(좌우)
      - 카메라 y(세로) → 로봇 x(전후, 위로 갈수록 멀리=값 큼)
    이 대응 자체가 '좌표축이 다르다'를 보여 주는 학습 포인트다.
    """
    ws = safety.WORKSPACE                 # 단일 출처 작업영역(mm)
    x_lo, x_hi = ws["x"]                  # 로봇 전후 범위
    y_lo, y_hi = ws["y"]                  # 로봇 좌우 범위
    m = 10.0                              # 경계 충돌 방지용 안전 마진(mm)
    rx_lo, rx_hi = x_lo + m, x_hi - m     # 마진 적용한 로봇 x 사용범위
    ry_lo, ry_hi = y_lo + m, y_hi - m     # 마진 적용한 로봇 y 사용범위

    # (카메라픽셀, 로봇mm) 네 모서리 대응 — y축은 화면 위(작은 py)가 로봇 먼쪽(큰 rx)
    return [
        ((0.0,       0.0),       (rx_hi, ry_lo)),  # 좌상 → 멀고 왼쪽
        ((frame_w,   0.0),       (rx_hi, ry_hi)),  # 우상 → 멀고 오른쪽
        ((0.0,       frame_h),   (rx_lo, ry_lo)),  # 좌하 → 가깝고 왼쪽
        ((frame_w,   frame_h),   (rx_lo, ry_hi)),  # 우하 → 가깝고 오른쪽
    ]


# ===========================================================================
# 5) 텔레오퍼레이션 루프 — 손 추적 → 픽셀 → 아핀 → 로봇 이동
# ===========================================================================
def teleop(M, robot, tracker, cam, frames=8, z_plane=20.0, verbose=True):
    """프레임을 받아 손 포인터를 로봇 좌표로 사상하고 로봇을 이동시킨다.

    인자
      M: 아핀행렬(estimate_affine 결과).
      robot: RobotController(연결됨).
      tracker: HandTracker.
      cam: Camera.
      frames: 처리할 프레임 수(데모는 유한 루프 — 무한루프 금지, 안전 스캔 통과).
      z_plane: 텔레오퍼레이션 평면 높이(mm). 손은 x/y만, z는 고정 평면.
      verbose: 진행 로그 출력 여부.
    반환
      처리 기록 리스트 [{frame, pixel, robot_xy, move_result}, ...]
    """
    history = []                          # 각 프레임 결과 기록(검수/디버그용)
    for i in range(frames):               # 유한 루프 — '정지 조건 있는' 안전한 루프
        frame = cam.read()                # mock 합성 프레임 1장
        # 프레임 크기(없으면 기본값) — 픽셀 환산에 필요
        if hasattr(frame, "shape"):
            fh, fw = frame.shape[0], frame.shape[1]
        else:
            fh, fw = 480, 640

        hand = tracker.process(frame)     # 손 추적 → {landmarks, handedness} 또는 {}
        pixel = hand_to_pixel(hand.get("landmarks"), fw, fh)  # 검지끝 픽셀

        if pixel is None:                 # 손이 안 잡힌 프레임은 건너뜀
            if verbose:
                print(f"  [{i+1}/{frames}] 손 미검출 → 이동 생략")
            history.append({"frame": i, "pixel": None,
                            "robot_xy": None, "move_result": None})
            continue

        rx, ry = apply_affine(M, pixel)   # 카메라 점 → 로봇 (x,y) mm

        # 1차 안전 확인(보고용) — 실제 보정은 move_to 내부 clamp 가 담당
        ok, msg = safety.check_move(rx, ry, z_plane)
        # 로봇 이동(절대좌표). 범위 밖이면 내부에서 클램프+경고.
        res = robot.move_to(rx, ry, z_plane, r=0.0, wait=True)

        if verbose:
            tag = "안전" if ok else f"보정필요({msg})"
            print(f"  [{i+1}/{frames}] 픽셀({pixel[0]:.0f},{pixel[1]:.0f}) "
                  f"→ 로봇({rx:.1f},{ry:.1f},z={z_plane}) [{tag}] "
                  f"clamped={res['clamped']}")

        history.append({"frame": i, "pixel": pixel,
                        "robot_xy": (rx, ry), "move_result": res})
    return history


# ===========================================================================
# 6) 자가 데모 (mock) — 하드웨어/키 없이 끝까지 실행
# ===========================================================================
def main():
    print("=" * 64)
    print(" H2 텔레오퍼레이션 — 좌표계 선형 변환 [solution 데모, mock]")
    print("=" * 64)

    if not _HAS_NUMPY:                    # numpy 필수 차시 — 없으면 안내 후 종료
        print(" numpy가 없어 데모를 건너뜁니다. `pip install numpy` 후 실행하세요.")
        return

    # 1) 캘리브레이션: 대응점 → 아핀행렬 추정
    print("\n[1] 캘리브레이션 (대응점 → 아핀행렬 추정)")
    corr = default_correspondences(640, 480)         # 코너 4점 대응표
    for (c, r) in corr:                              # 학생이 입력할 대응점 출력
        print(f"  카메라{tuple(round(v,1) for v in c)} ↔ "
              f"로봇{tuple(round(v,1) for v in r)}")
    M = estimate_affine(corr)                        # 최소제곱 추정
    print("  추정된 아핀행렬 M(2x3):")
    for row in M:
        print("   ", "  ".join(f"{v:+8.4f}" for v in row))

    # 2) 추정 검증: 대응점을 다시 넣어 재투영 오차 확인(작아야 함)
    print("\n[2] 재투영 오차 점검(대응점 자체)")
    max_err = 0.0
    for (c, r) in corr:
        pr = apply_affine(M, c)                      # 추정행렬로 되사상
        err = ((pr[0]-r[0])**2 + (pr[1]-r[1])**2) ** 0.5
        max_err = max(max_err, err)
    print(f"  최대 재투영 오차: {max_err:.4f} mm (0에 가까울수록 좋음)")

    # 3) 텔레오퍼레이션: 손추적 → 픽셀 → 로봇 이동 (mock 자원)
    print("\n[3] 텔레오퍼레이션 루프 (mock 손추적 → 로봇 이동)")
    with get_robot(mock=True) as robot:              # MockDobot, with 종료시 안전정지
        robot.home()                                 # 전원 직후 원점복귀 습관(API 8절)
        tracker = HandTracker(mock=True)             # mock 손추적기(캔드 랜드마크)
        # H2 텔레오퍼레이션은 사용자 손을 추적하는 '손 카메라'(config.CAM_HAND)를 쓴다.
        #   (데모는 mock 합성 프레임이라 실제로 열지 않지만, 실모드 전환 시 의도가 명확하도록 인덱스를 명시한다.)
        with Camera(source=config.CAM_HAND, mock=True) as cam:  # 손 카메라(데모=mock 합성 프레임)
            hist = teleop(M, robot, tracker, cam, frames=5, z_plane=20.0)

    # 4) 요약
    moved = [h for h in hist if h["move_result"] is not None]
    print(f"\n[4] 요약: 총 {len(hist)}프레임 중 {len(moved)}프레임에서 로봇 이동 수행.")
    print(" 데모 완료 — 하드웨어 없이 mock 으로 캘리브레이션→매핑→텔레오퍼레이션 실행됨.")
...
# ==============================================================================
# ── h2_teleop/gui.py — H2 화면/조작 로직 + main()
# ==============================================================================

"""H2 · 고등부(Python) — 텔레오퍼레이션 + 제스처 전(全)기능 제어 GUI.

손 하나로 두봇 라이트의 모든 동작을 제어한다(마우스 버튼 보조).
  ☝ 가리키기(point)  : 손 위치 → 로봇 X/Y 이동(아핀 변환)
  ✋ 손펴기(open)     : Z 상승(들어올리기)
  ✊ 주먹(fist)       : Z 하강(내리기/집기 높이까지)
  🤏 핀치(pinch)     : 그리퍼 열기/닫기 토글
  [버튼] R 회전 ±, 그리퍼 토글, 홈 복귀, 텔레옵 시작/정지, 재캘리브레이션

solution.py 의 순수함수를 재사용: estimate_affine / default_correspondences /
hand_to_pixel / apply_affine.

성능: mediapipe 입력을 다운스케일해 처리(끊김↓). 화면 표시는 베이스가 캡처/처리와
분리해 부드럽게 갱신한다.

규칙: 로봇 동작 버튼은 threading 으로(UI 안 막음). 제스처 이동은 rate-limit + wait=False.
GUI 생성/실행은 __main__ 아래에서만.
"""
...
# ── 제어 파라미터(쉽게 조정) ─────────────────────────────────────────────
Z_INIT = 50.0          # 텔레옵 시작 시 기본 z(mm)
Z_MIN = float(getattr(config, "GRASP_Z", -25))   # 최저(집기 높이)
Z_MAX = 130.0          # 최고
Z_STEP = 5.0           # open/fist 한 틱당 z 변화(mm)
R_STEP = 15.0          # R 회전 버튼 한 번당 각도(deg)
MOVE_INTERVAL = 0.30   # 로봇 이동 최소 간격(s)
DEADBAND_MM = 8.0      # X/Y 미세 떨림 무시(mm)
PINCH_THRESH = 0.06    # 핀치 판정(정규화 좌표상 엄지끝~검지끝 거리)
DOWNSCALE = 0.5        # mediapipe 입력 축소 비율(0.5=절반 → 약 2배 빠름)
EXT_MARGIN = 1.05      # 손가락 펴짐 판정 여유
PROCESS_EVERY_N = 5    # N프레임당 1회만 손 인식(mediapipe) 수행 → 끊김↓·CPU↓(라이브 조정 가능)


# ── 제스처 분류(방향 무관: 손목 기준 거리) ────────────────────────────────
def _pt(lms, i):
    p = lms[i]
...
def classify_control_gesture(lms):
    """손 랜드마크 → 'point'|'open'|'fist'|'pinch'|'other'|'none'."""
    if not lms or len(lms) < 21:
        return "none"
    wrist = _pt(lms, 0)

    def ext(pip_i, tip_i):  # 손목~끝 > 손목~중간마디 → 펴짐(회전 무관)
        return _d(wrist, _pt(lms, tip_i)) > _d(wrist, _pt(lms, pip_i)) * EXT_MARGIN

    idx, mid = ext(6, 8), ext(10, 12)
    rng, pky = ext(14, 16), ext(18, 20)
    up = sum((idx, mid, rng, pky))

    if _d(_pt(lms, 4), _pt(lms, 8)) < PINCH_THRESH:   # 엄지끝~검지끝 가까움
        return "pinch"
    if up == 0:
        return "fist"
    if up >= 4:
        return "open"
    if idx and not rng and not pky:
        return "point"
    return "other"


_G_KO = {"point": "가리키기→XY", "open": "펴기→Z↑", "fist": "주먹→Z↓",
         "pinch": "핀치→그리퍼", "other": "대기", "none": "손없음"}


def _draw_trail(frame, trail):
    """검지끝 최근 경로를 점점 흐려지는 선으로(잔상)."""
    if cv2 is None or len(trail) < 2:
        return
    pts = list(trail)
    n = len(pts)
    for i in range(1, n):
        ...
        if p0 is None or p1 is None:
            continue
        f = i / n
        color = (0, int(70 + 185 * f), int(70 + 185 * f))
        cv2.line(frame, p0, p1, color, max(1, int(1 + 4 * f)), cv2.LINE_AA)
...
def make_app():
    ...
    state = {
        "M": None, "tracker": None, "teleop_on": False,
        "last_move": 0.0, "trail": deque(maxlen=20),   # 스로틀로 점이 드물어 maxlen↓(최근 궤적만)
        "x": None, "y": None, "z": Z_INIT, "r": 0.0,  # 현재 로봇 목표
        "grip": False, "pinch_prev": False,
        # 프레임 스로틀: N프레임당 1회만 인식(mediapipe). 사이 프레임은 마지막 결과로 표시만.
        "frame_i": 0, "process_every": PROCESS_EVERY_N,
        "last_pixel": None, "last_g": "none",
    }

    def _ensure_calib(w, h, app):
        if state["M"] is None:
            state["M"] = solution.estimate_affine(solution.default_correspondences(w, h))
            app.log(f"[캘리브레이션] {w}x{h} 기준 아핀행렬 M 계산 완료.")

    def _send_move(app):
        """현재 state(x,y,z,r)로 로봇 이동(rate-limit). x/y 미설정이면 생략."""
        if state["x"] is None or app.robot is None:
            return
        now = time.time()
        ...
        state["last_move"] = now
        try:
            app.robot.move_to(state["x"], state["y"], state["z"], r=state["r"], wait=False)
        except Exception as e:
    ...
    def process_frame(frame, app):
        ...
        _ensure_calib(w, h, app)
        if state["tracker"] is None:
            state["tracker"] = HandTracker(mock=False)
            try:
                app.log(f"[비전 백엔드] {state['tracker'].backend_info()}")
            except Exception:
                pass

        # ── 프레임 스로틀 ─────────────────────────────────────────────────
        #   N프레임당 1회만 손 인식(mediapipe.process)을 수행한다(끊김↓·CPU↓).
        #   스킵 프레임: '새 카메라 영상' + '마지막 인식결과(커서/잔상)'로 표시만 갱신한다.
        #     → 영상 '배경'은 매 프레임 갱신돼 부드럽고, 손 인식표시(커서)는 N프레임마다 갱신된다.
        #   로봇 명령은 인식 프레임에서만(스킵 프레임이 stale 제스처로 오작동하지 않게).
        #   주의: 이 카운터는 '워커가 실제 처리한 새 프레임' 기준이라 CPU 부하에 따라
        #         체감 인식속도가 변한다(요청대로 프레임 기준; 시간 기준 아님).
        state["frame_i"] += 1
        n = max(1, int(state["process_every"]))
        do_recognize = (state["frame_i"] % n == 0)

        if do_recognize:
            # 성능: 다운스케일한 프레임으로 손 추적(정규화 좌표라 풀해상도에 그대로 사용)
            small = cv2.resize(frame, (0, 0), fx=DOWNSCALE, fy=DOWNSCALE) if DOWNSCALE != 1.0 else frame
            lms = state["tracker"].process(small).get("landmarks")
            pixel = solution.hand_to_pixel(lms, w, h)
            g = classify_control_gesture(lms) if lms else "none"
            state["last_pixel"], state["last_g"] = pixel, g   # 스킵 프레임 표시에 재사용
        else:
            pixel, g = state["last_pixel"], state["last_g"]   # 마지막 인식결과 재사용
        on = state["teleop_on"]

        # 상단 상태 오버레이(스로틀 주기 표시 포함)
        cv2.putText(frame, ("TELEOP ON" if on else "TELEOP OFF") + f"  [{_G_KO.get(g, g)}]  1/{n}f",
                    (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                    (0, 255, 0) if on else (0, 0, 255), 2)
        cv2.putText(frame, f"z={state['z']:.0f} r={state['r']:.0f} grip={'CLOSE' if state['grip'] else 'OPEN'}",
                    (10, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

        if pixel is None:
            # 손이 사라지면 잔상을 '빠르게' 감쇠한다. (인식 프레임이 N배 드물어졌으므로
            # 1점씩 빼면 잔상이 한참 남아 '유령 궤적'처럼 보임 → 인식 프레임당 여러 점 제거)
            if do_recognize:
                for _ in range(3):
                    if state["trail"]:
                        state["trail"].popleft()
        ...
        if do_recognize:                          # 새 잔상점은 인식 프레임에서만 추가
            state["trail"].append((px, py))
        _draw_trail(frame, state["trail"])
        cv2.circle(frame, (px, py), 10, (0, 255, 255), 2)
        cv2.circle(frame, (px, py), 2, (0, 255, 255), -1)

        # 로봇 동작은 '인식 프레임'에서만(스킵 프레임은 표시만 — stale 제스처 재실행 방지)
        if not do_recognize:
            return frame
        if not (on and app.robot is not None):
            return frame

        # ── 제스처 → 동작 ────────────────────────────────────────────────
        if g == "pinch":
            if not state["pinch_prev"]:           # 상승에지에서만 1회 토글
                new = not state["grip"]
                state["grip"] = new
                try:
                    app.robot.grasp(new)
                    app.log("그리퍼 " + ("닫음(잡기)" if new else "엶(놓기)"))
                except Exception as e:
                    app.log(f"[그리퍼 오류] {e}")
            state["pinch_prev"] = True
            return frame
        state["pinch_prev"] = False

        if g == "point":
            rx, ry = solution.apply_affine(state["M"], pixel)
            if (state["x"] is None or abs(rx - state["x"]) >= DEADBAND_MM
                    or abs(ry - state["y"]) >= DEADBAND_MM):
                state["x"], state["y"] = rx, ry
                _send_move(app)
                app.set_status(f"이동 X/Y=({rx:.0f},{ry:.0f}) z={state['z']:.0f}")
        elif g in ("open", "fist"):
            # Z는 '상대 누적'이라, 인식 프레임마다 +5씩 더하면 이동제한(MOVE_INTERVAL)에 걸려
            # 화면 z 값만 앞서가고 팔은 15~20mm씩 튄다(게다가 N이 작을수록 더 심함 — 반직관).
            # → z 증감을 이동 간격에 맞춰 '한 틱(Z_STEP)씩'만 진행 → 프레임수/ N 과 무관하게
            #   일정한 상승/하강 속도. (point 의 X/Y 는 절대좌표라 self-correct 되므로 영향 없음)
            if time.time() - state["last_move"] >= MOVE_INTERVAL:
                if g == "open":
                    nz = min(state["z"] + Z_STEP, Z_MAX)
                else:
                    nz = max(state["z"] - Z_STEP, Z_MIN)
                if nz != state["z"]:
                    state["z"] = nz
                    _send_move(app)   # 위 조건으로 간격이 지났으므로 실제로 이동함
                    app.set_status((("Z 상승 → " if g == "open" else "Z 하강 → ") + f"{nz:.0f}"))
        return frame

    # ── 버튼 콜백(로봇 동작은 스레드로) ──────────────────────────────────
    def _seed_from_pose(app):
        """현재 로봇 포즈로 state(x,y,z,r) 초기화(제스처가 현재 위치에서 출발하도록)."""
        try:
            p = app.robot.get_pose()
            state["x"], state["y"], state["z"], state["r"] = p["x"], p["y"], p["z"], p["r"]
        except Exception:
            pass

    def toggle_teleop(app):
        def worker():
            if state["teleop_on"]:
                state["teleop_on"] = False
                app.set_status("텔레옵 정지됨"); app.log("텔레옵 정지.")
            else:
                if app.robot is None:
                    app.log("[안내] 먼저 '로봇 연결'을 누르세요."); return
                _seed_from_pose(app)
                state["teleop_on"] = True
                state["last_move"] = 0.0
                app.set_status("텔레옵 시작 — ☝이동 ✋Z↑ ✊Z↓ 🤏그리퍼")
                app.log("텔레옵 시작: 가리키기=XY, 펴기=Z↑, 주먹=Z↓, 핀치=그리퍼.")
        threading.Thread(target=worker, daemon=True).start()

    def recalibrate(app):
        state["M"] = None
        app.log("[캘리브레이션] 다음 프레임에서 M 재계산."); app.set_status("재캘리브레이션 예약")

    def rotate(app, delta):
        def worker():
            if app.robot is None:
                app.log("[안내] 로봇 미연결."); return
            if state["x"] is None:
                _seed_from_pose(app)
            state["r"] = max(-150.0, min(150.0, state["r"] + delta))
            try:
                app.robot.move_to(state["x"], state["y"], state["z"], r=state["r"], wait=True)
                app.log(f"R 회전 → {state['r']:.0f}°"); app.set_status(f"R={state['r']:.0f}°")
            except Exception as e:
                app.log(f"[회전 오류] {e}")
        threading.Thread(target=worker, daemon=True).start()
    ...
    def grip_toggle(app):
        def worker():
            if app.robot is None:
                app.log("[안내] 로봇 미연결."); return
            new = not state["grip"]; state["grip"] = new
            try:
                app.robot.grasp(new)
                app.log("그리퍼 " + ("닫음(잡기)" if new else "엶(놓기)"))
            except Exception as e:
    ...
    def go_home(app):
        def worker():
            if app.robot is None:
                app.log("[안내] 로봇 미연결."); return
            try:
                app.robot.home(); _seed_from_pose(app)
                app.log("홈 복귀 완료."); app.set_status("홈 복귀")
            except Exception as e:
                app.log(f"[홈 오류] {e}")
        threading.Thread(target=worker, daemon=True).start()

    def build_controls(parent, app):
        """인식 주기(스로틀 N) 라이브 조정 — 끊김/반응성 균형을 직접 맞추도록."""
        import tkinter as tk
        tk.Label(parent, text="인식 주기 (N프레임당 1회 인식)", bg="#2a2a3c", fg="#9ecbff",
                 font=("맑은 고딕", 10, "bold")).pack(anchor="w", pady=(10, 2))
        row = tk.Frame(parent, bg="#2a2a3c")
        row.pack(fill="x")
        var = tk.StringVar(value=str(state["process_every"]))

        def set_n(v):
            state["process_every"] = int(v)
            state["frame_i"] = 0   # 위상 리셋 → 변경한 주기가 바로 깔끔하게 적용
            app.log(f"인식 주기 = {v} 프레임당 1회 (작을수록 즉각·무거움 / 클수록 부드럽지만 반응 느림)")

        for v in (1, 3, 5, 8, 10):
            tk.Radiobutton(row, text=str(v), value=str(v), variable=var,
                           command=lambda x=v: set_n(x), bg="#2a2a3c", fg="#e6e6e6",
                           selectcolor="#16161f", font=("맑은 고딕", 9)).pack(side="left")

    info = ("손 카메라에 손을 보이고 '로봇 연결' → '텔레옵 시작'. "
            "☝가리키기=X/Y · ✋펴기=Z↑ · ✊주먹=Z↓ · 🤏핀치=그리퍼. "
            "R 회전·홈은 버튼. 좌표 어긋나면 '재캘리브레이션'. "
            f"인식은 {PROCESS_EVERY_N}프레임당 1회(우측에서 조정).")

    app = LessonGUI(
        title="H2 텔레오퍼레이션 (제스처 전기능 제어)",
        camera_index=config.CAM_HAND,
        use_robot=True,
        process_frame=process_frame,
        controls=[
            ("텔레옵 시작/정지", toggle_teleop),
            ("그리퍼 열기/닫기", grip_toggle),
            ("R 회전 +15°", lambda a: rotate(a, R_STEP)),
            ("R 회전 -15°", lambda a: rotate(a, -R_STEP)),
            ("홈 복귀", go_home),
            ("재캘리브레이션", recalibrate),
        ],
